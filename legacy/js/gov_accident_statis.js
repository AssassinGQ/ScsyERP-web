...
function _tran_load(d) {
    ...
    for(var i = 0; i < d.length; i++){
        d[i]["warntype"] = tran_warntype(d[i]["warntype"]);
...
function _parmfun() {
    ...
    if(min.length != 0 || max.length != 0){
        var warntime = {};
        if(min.length != 0){
            warntime["min"] = new Date(min).getTime();
        }
        if(max.length != 0){
            warntime["max"] = new Date(max).getTime();
        }
        param["warntime"] = JSON.stringify(warntime);
    }
    return param;
}
function _search() {
...
function tran_load(d) {
    ...
    for(var i = 0; i < d.length; i++){
        ...
        var total = d[i].overspeedamount + d[i].parkamount + d[i].suddenbrakeamount +
